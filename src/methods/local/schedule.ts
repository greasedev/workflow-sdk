...
const taskStore: Map<string, SchedulerTaskInfo & { job: SchedulerJob }> = new Map()
...
export class Scheduler {
  ...
  async status(): Promise<SchedulerStatusResult> {
    ...
    const nextWakeAtMs = tasks.reduce(
      (min: number | null, t) => t.nextRunAtMs && (min === null || t.nextRunAtMs < min) ? t.nextRunAtMs : min,
      null,
    )
    return {
      running: true,
  ...
  async add(job: SchedulerJob): Promise<SchedulerAddResult> {
    ...
    const task: SchedulerTaskInfo & { job: SchedulerJob } = {
      id,
      name: job.name,
  ...
  async update(taskId: string, patch: SchedulerPatch): Promise<SchedulerUpdateResult> {
    ...
    if (patch.deleteAfterRun !== undefined) task.job.deleteAfterRun = patch.deleteAfterRun
    if (patch.timeoutMs !== undefined) task.job.timeoutMs = patch.timeoutMs
    ...
    if (patch.payload !== undefined) {
      ...
      if (patchPayload.kind !== currentPayload.kind) {
        ...
        if (patchPayload.kind === 'agentTurn' && currentPayload.kind === 'agentTurn') {
          if (patchPayload.message !== undefined) currentPayload.message = patchPayload.message
          if (patchPayload.model !== undefined) currentPayload.model = patchPayload.model
          if (patchPayload.timeoutMs !== undefined) currentPayload.timeoutMs = patchPayload.timeoutMs
        } else if (patchPayload.kind === 'chatInject' && currentPayload.kind === 'chatInject') {
          if (patchPayload.chatId !== undefined) currentPayload.chatId = patchPayload.chatId
          if (patchPayload.message !== undefined) currentPayload.message = patchPayload.message
        } else if (patchPayload.kind === 'invokeWorkflow' && currentPayload.kind === 'invokeWorkflow') {
          if (patchPayload.workflowName !== undefined) currentPayload.workflowName = patchPayload.workflowName
          if (patchPayload.workflowParams !== undefined) currentPayload.workflowParams = patchPayload.workflowParams
        }
      }
  ...
  private extractPayloadInfo(payload: SchedulePayload): { kind: string; message?: string } {
  ...
  private calculateNextRunAtMs(schedule: SchedulerJob['schedule'], now: number): number | undefined {
    switch (schedule.kind) {
      case 'at':
        if (schedule.atMs) return schedule.atMs
